...
----
mkcert -install
mkcert vaultwarden.lan vaultwarden-lan 192.168.178.50
----
  ...
  * Root-CA im Betriebssystem installiert (durch mkcert install)

. FritzBox DNS-Rebind anpassen
  Lege den Hostnamen intern fest:

  * FritzBox → *Heimnetz* → *Netzwerk*
  * Deinem Unraid-Server einen Namen geben, z. B. `vaultserver.lan` oder `vaultserver.local`
  * Meist bereitet `.local` Probleme, daher besser `.lan` verwenden (`vaultserver.lan`)
...
ROCKET_TLS={certs="/data/ssl/vaultwarden.lan.pem",key="/data/ssl/vaultwarden.lan-key.pem"}
----

  Die Zertifikate vorher nach
  `/mnt/user/appdata/vaultwarden/ssl` kopieren.

. Im Browser-Plugin Hintergrund-Domain ändern
  In den Bitwarden-Plugin-Einstellungen:

  * „Selbstsigniertes Zertifikat zulassen“
  * URL setzen auf `https://vaultwarden-lan`  (FritzBox Eigenheit sonst https://vaultwarden.lan)
...
.Nachteile
* Root-CA (rootCA.pam) muss auf allen Geräten importiert werden (PC, Handy usw.)

(Auf dem Rechner wo die Zertifikate erzeugt wurden ausführen von `mkcert -CAROOT` zeigt in welchem Folder die rootCA.pem liegt)
